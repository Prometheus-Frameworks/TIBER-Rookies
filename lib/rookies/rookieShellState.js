...
import { getRookieStubLoadState, filterRookieCardsByStubCoverage, findRookieStubBySlug } from './rookieStubs.js';

export async function getRookieShellState() {
  const [alpha, stubResult] = await Promise.all([
    getRookieCardLoadState(),
    getRookieStubLoadState(),
  ]);
...
export function buildRookieShellState(alpha, stubResult) {
  ...
  const cards = filterRookieCardsByStubCoverage(alpha.cards, stubResult);
