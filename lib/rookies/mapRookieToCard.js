...
function toSlug(playerId) { return playerId; }
...
export function mapRookieToCard({ alphaPlayer, combineRow, productionRow, draftRow, rank }) {
  ...
  const tags = [
    ...
    draftCapital != null && draftCapital >= 75 ? 'Draft capital-friendly profile' : 'Later-capital profile',
  ].filter(Boolean);

  return {
    playerId: alphaPlayer.player_id,
    slug: toSlug(alphaPlayer.player_id),
    identity: {
