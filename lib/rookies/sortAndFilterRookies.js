...
export function sortAndFilterRookies(cards, { sort = 'grade', position = 'ALL', tag = 'ALL', nameFilter = '' } = {}) {
  ...
  const filtered = cards.filter((card) => {
    ...
    const matchesName = !nameLower || card.identity.name.toLowerCase().includes(nameLower);
    return matchesPosition && matchesTag && matchesName;
  });
