...
- `size_context_0_100` (number | null): deterministic height/weight percentile context signal for talent-profile tie-breaking. For WR, computed as `max(0, min(100, round(50 + composite_z * 15, 1)))` where `composite_z = 0.55 * height_z + 0.45 * weight_z`, `height_z = (height_in - 72.5) / 2.0`, `weight_z = (weight_lb - 197.0) / 18.0`. Reference anchors (72.5 in / 197 lb) are approximate NFL combine WR medians. For TE, computed as `max(0, min(100, round(50 + composite_z * 15, 1)))` where `composite_z = 0.55 * height_z + 0.45 * weight_z`, `height_z = (height_in - 76.5) / 2.5`, `weight_z = (weight_lb - 252.0) / 20.0`. Reference anchors (76.5 in / 252 lb) are approximate NFL combine TE medians. Historical WR/TE rows were seeded from the same ingredient set; 2026 rookie WR/TE scores are derived from `data/raw/2026_combine_results.json`.
