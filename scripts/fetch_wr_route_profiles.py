...
def summarize_player(player: dict[str, Any], plays: list[dict[str, Any]], season_type: str) -> dict[str, Any]:
    ...
    team_pass_plays = [play for play in offense_plays if play_type_of(play) in PASS_PLAY_TYPES]
    team_screen_plays = [play for play in team_pass_plays if parse_screen_flag(play_text_of(play))]

    targeted: list[dict[str, Any]] = []
    for play in team_pass_plays:
        if is_targeted_player(play_text_of(play), str(player.get("player_name", ""))):
            targeted.append(play)

    screen_targets = [play for play in targeted if parse_screen_flag(play_text_of(play))]
    depth_tags = [parse_depth_tag(play_text_of(play)) for play in targeted]
    tagged_depths = [tag for tag in depth_tags if tag is not None]

    targets = len(targeted)
    receptions = sum(1 for play in targeted if play_type_of(play) in COMPLETION_PLAY_TYPES)
    yards_total = sum(yards_gained_of(play) for play in targeted)
    team_yards_sum = sum(yards_gained_of(play) for play in team_pass_plays)
